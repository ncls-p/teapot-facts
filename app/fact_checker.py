...
class TeapotFactChecker:
    ...
    def __init__(self, log_level=logging.INFO):
        ...
        # We'll store document info separately for document-based RAG
    ...
    def check_fact(
        ...
        try:
            # Process documents if provided (for RAG)
            if documents:
                return self._process_with_documents(query, documents)

            # Process with direct context if provided
            elif context:
                return self._process_with_context(query, context)

            # Process without context (more likely to get a hallucination refusal)
            else:
    ...
    def _process_with_documents(
        ...
        # Store documents for source tracking
        self.stored_documents = []
        ...
        # Add documents to the model's RAG system
        # Since the model doesn't directly support document addition through an API,
        # we'll build a combined context from all documents
        combined_context = "\n\n".join(
            [
                doc.get("content", "")
        ...
        # Query with the combined context
    ...
    def _process_with_context(self, query: str, context: str) -> Dict[str, Any]:
        ...
        # Direct query with context as per TeapotAI documentation
    ...
    def _process_without_context(self, query: str) -> Dict[str, Any]:
        ...
        # According to documentation, TeapotLLM is trained to refuse when context is insufficient
    ...
    def extract_information(
        ...
        try:
            ...
            # Use the extract method according to TeapotAI documentation
            if query:
                # If query is provided, use it to focus the extraction
                logger.debug(f"Extracting with query: {query}")
                extracted_data = self.model.extract(
                    model_class, context=context, query=query
                )
            else:
                # Otherwise just extract from the context
    ...
    def _get_sources(self) -> List[Dict[str, Any]]:
        ...
        # First use our stored documents
        ...
        # If no stored documents, try to access documents from model if available
        if not sources:
            logger.debug("No stored documents, attempting to retrieve from model")
            try:
                # Check if the model has documents property
